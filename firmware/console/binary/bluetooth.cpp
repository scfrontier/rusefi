...
static void runCommands(SerialTsChannelBase* tsChannel) {
	...
	if(workingBaudIndex == (uint8_t)efi::size(baudRates))//failed
...
uint8_t findBaudIndex(SerialTsChannelBase* tsChannel) {
	...
	for(uint8_t baudIdx=0; baudIdx < efi::size(baudRates); baudIdx++) {
		...
		if (baudIdx == efi::size(baudRates)) {
			...
			return (uint8_t)efi::size(baudRates);//failed
		}
...
//allow user to manually send commands to bluetooth module for discovery purposes
void bluetoothCmd(bluetooth_module_e moduleType,const char *cmd){
	btModuleType = moduleType;
	strncpy(btcmd, cmd, 40);
